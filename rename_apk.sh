#!/bin/bash
# flutter build apk --release 후 APK 파일명을 Daily_Prayer-{버전}-release.apk 형식으로 변경
...
for apk in "$APK_DIR"/app-*.apk; do
    ...
    # app-release.apk -> Daily_Prayer-1.0.0-release.apk
    newname="${filename/app-/Daily_Prayer-${VERSION}-}"
